            ...
            for phase in procedure:
                ...
                with torch.set_grad_enabled(phase == 'trainLabeled' or phase == 'trainUnlabeled'):
                    ...
                    if phase == 'trainLabeled':
                        ...
                        # Error here in the original code, the solution in this Issue ticket solves it
                        # https://github.com/ShuaiChenBIGR/MASSL-segmentation-framework/issues/1
                        # loss = w2 * criterion[1](outputsU_back.float(), labels_back.float()) + \
                        #        w3 * criterion[1](outputsU_fore.float(), labels_fore.float())
                        loss = w2 * criterion[1](outputsU_back.detach().float(), labels_back.detach().float()) + \
                           w3 * criterion[1](outputsU_fore.detach().float(), labels_fore.detach().float())
